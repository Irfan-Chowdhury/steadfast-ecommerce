...
<div class="container">
    ...
    <form method="POST" action="{{ route('sales.store') }}">
        ...
        <div class="form-group">
            ...
            <input type="date" name="sale_date" class="form-control" required value="{{ date('Y-m-d',strtotime($sale->date)) }}">
            @error('sale_date')
                        ...
                        <td>
                            <select name="products[0][product_id]" class="form-control product-dropdown">
                                ...
                                @foreach($products as $product)
                                    <option {{ $saleItem->product_id === $product->id ? 'selected' : ''  }} value="{{ $product->id }}" data-price="{{ $product->sell_price }}">{{ $product->name }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td><input type="number" name="products[0][quantity]" value="{{ $saleItem->quantity }}" class="form-control qty" required></td>
                        <td><input type="number" step="0.01" name="products[0][unit_price]" value="{{ $saleItem->unit_price }}" class="form-control price" readonly></td>
                        <td><input type="text" class="form-control total"  value="{{ $saleItem->quantity * $saleItem->unit_price }}" readonly></td>
        ...
        <div class="form-group">
            ...
            <input type="number" name="discount" value="{{ $sale->discount }}" class="form-control" value="0">
            @error('discount')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
        ...
        <div class="form-group">
            <label>VAT (%)</label>
            <input type="number" name="vat_percent" value="{{ $sale->vat_percentage}}" class="form-control" value="0">
            @error('vat_percent')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="paid_amount" value="{{ $sale->paid_amount}}" class="form-control">
            @error('paid_amount')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="due_amount" value="{{ $sale->due_amount}}" class="form-control" readonly>
        </div>
